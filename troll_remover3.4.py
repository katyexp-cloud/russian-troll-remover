...
class AIApp:
    ...
    def load_config(self):
        # Default configuration with your exact offsets and waits
        default_config = {
            ...
            "waits": {
                ...
                "copy_delay": 0.3, 
                "action_shift": 1.0, 
                "human_move_short": [0.2, 0.4], 
                "human_move_med": [0.3, 0.5], 
                "human_move_drag": [0.4, 0.6],
                "human_move_click": [0.25, 0.45], 
                "post_click": [0.6, 0.9],         
                "target2_interval": [0.4, 0.7],   
                "between_targets": [0.5, 0.8]
            }
        }
        ...
        # Save if not exists or corrupted
    ...
    def main_logic(self):
        try:
            # Initialize AI Classifier if not already loaded
            ...
            while self.is_running:
                ...
                if not targets:
                    self.write_log("✅ Žádné cíle nebyly nalezeny. Zkouším scroll.")
                    pyautogui.scroll(self.config["offsets"]["scroll_amount"])
                ...
                for t in targets:
                    ...
                    # Calculate center and base offset point
                    cx, cy = pyautogui.center(t)
                    offset_x = cx + self.config["offsets"]["analyze_x"] # center_x - 20
                    offset_y = cy + self.config["offsets"]["analyze_y"] # center_y + 20
                    
                    self.write_log(f"📍 Cíl: ({offset_x}, {offset_y})")
                    
                    # Initial move to target area
                    self.human_move(offset_x, offset_y, self.r_time(*self.config["waits"]["human_move_short"]))
                    
                    # Drag sequence to highlight text
                    # Move to start: offset_x + 20, offset_y - 5
                    self.human_move(offset_x + self.config["offsets"]["drag_start_x"],
                                    offset_y + self.config["offsets"]["drag_start_y"], 
                                    self.r_time(*self.config["waits"]["human_move_med"]))
                    
                    pyautogui.mouseDown()
                    # Move to end: offset_x - 400, offset_y
                    self.human_move(offset_x + self.config["offsets"]["drag_end_x"],
                                    offset_y + self.config["offsets"]["drag_end_y"], 
                                    self.r_time(*self.config["waits"]["human_move_drag"]))
                    pyautogui.mouseUp()
                    ...
                    action = False
                    # Check Blacklist, Cache, and AI classification
                    ...
                    if action:
                        # Perform the deletion/target sequence if AI flagged the text
                ...
                # After processing all targets on screen, scroll down
                self.write_log("✅ Všechny cíle na obrazovce zpracovány. Scrollování...")
                last_target = targets[-1]
                lx, ly = pyautogui.center(last_target)
                self.human_move(lx, ly, self.r_time(0.3, 0.5))
                pyautogui.scroll(self.config["offsets"]["scroll_amount"])
                time.sleep(1.0) # Wait for scroll animation

        except Exception as e:
            self.write_log(f"❌ Chyba: {str(e)}")
    ...
    def main_logic(self):
        try:
            ...
            while self.is_running:
                self.write_log("Skenuji...")
                targets = list(pyautogui.locateAllOnScreen('target.png', confidence=0.7))
                
                if not targets:
                    self.write_log("Skroluji...")
                ...
                action_taken_on_screen = False
                for t in targets:
                    if not self.is_running: break
                    
                    cx, cy = pyautogui.center(t)
                    off_x, off_y = cx - 10, cy - 10
                    
                    self.write_log(f"Cíl: ({off_x}, {off_y})")
                    ...
                    self.write_log("400px left")
                    pyautogui.mouseDown()
                    self.human_move(off_x - 400, off_y + 30, self.r_time(0.3, 0.5))
                    pyautogui.mouseUp()
                    ...
                    if action:
                        self.execute_target_sequence(cx, cy)
                        time.sleep(1.0)
                        action_taken_on_screen = True
                        break
                    else:
                        self.write_log("OK.")
                        time.sleep(self.r_time(0.5, 0.8))

                if not action_taken_on_screen and self.is_running:
                    self.write_log("Výběr dokončen. Skroluji...")
